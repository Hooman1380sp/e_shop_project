...
class ContactUsView(APIView):
    ...
    def post(self, request: HttpRequest):
        ...
        if user is not None:
            ...
            message_contact = ContactUs(ip=ip_HTTP_X, title=title, user=user, full_name=full_name, email=email,
                                        message=message)
            ...
            email = ser_data.validated_data.get('email')
            message = ser_data.validated_data.get('message')
            message_contact = ContactUs(ip=ip_HTTP_X, title=title, full_name=full_name, email=email,
                                        message=message)
            message_contact.save()
            return Response(data=ser_data.data, status=status.HTTP_201_CREATED)
